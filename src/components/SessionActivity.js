...
const ReturnRefundManagement = () => {
  ...
  const handleStockUpdate = async (orderId) => {
    try {
      const response = await fetch(`/api/orders/${orderId}/update-stocks`, {
        method: 'PUT',
        headers: {
      ...
      setError('Failed to update product stocks. Please try again.');
    }
  };
  ...
  const handleStatusUpdate = async (orderId, status) => {
    try {
      if (status === 'Return Cancelled') {
        // Handle return cancellation with stock update
        const response = await fetch(`/api/orders/${orderId}/update-stocks`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
        ...
        if (!response.ok) {
          throw new Error('Failed to process return cancellation');
        }
      } else {
  ...
  return (
    <>
      <h2 className="text-2xl font-bold mb-6 text-gray-800">Return Requests</h2>
