...
const AddStocks = ({ isOpen, onClose, onSubmit }) => {
      ...
      if (formData.stockId || formData.productId) {
        try {
          ...
          if (formData.productId) {
            ...
            productExists = productResponse.data.products.length > 0;
            productDeleted = productExists && productResponse.data.products[0].deleted;
  
            if (productDeleted) {
              setProductError('This product has been deleted and cannot be used.');
              return;
            }
          ...
          // Check stock ID conflicts
          if (existingStock) {
            setStockIdExists(true);
            // If this Stock ID exists but with a different Product ID
            if (formData.productId && existingStock.product_id !== parseInt(formData.productId)) {
              setProductError(`Stock ID ${formData.stockId} is already assigned to Product ID ${existingStock.product_id}. You cannot use this Stock ID.`);
              return;
            }
          }
  
          // Check product ID conflicts
          if (productStock) {
            // If this Product ID already has a different Stock ID
            if (formData.stockId && productStock.id !== parseInt(formData.stockId)) {
              setProductError(`Product ID ${formData.productId} is already assigned to Stock ID ${productStock.id}. You cannot assign a different Stock ID.`);
              return;
            }
          }
  
          // Only set current stock if both IDs match exactly
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const productDeleted = productResponse.data.products.some(p => 
        p.id === parseInt(formData.productId) && p.deleted
      );
  
      if (productDeleted) {
        setError('Cannot add stock to a deleted product');
        setLoading(false);
        return;
      }
