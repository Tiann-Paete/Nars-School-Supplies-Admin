...
async function handleAddStock(req, res) {
  ...
  try {
    // Check product existence and deletion status
    ...
    if (productCheck[0].deleted) {
      return res.status(400).json({ error: 'Cannot add stock to a deleted product' });
    }

    // Check if the stock ID already exists with a different product
    const [existingStockCheck] = await db.query(
      'SELECT * FROM product_stocks WHERE id = ?',
      [id]
    );

    if (existingStockCheck.length > 0 && existingStockCheck[0].product_id !== product_id) {
      return res.status(400).json({ 
        error: `Stock ID ${id} is already assigned to Product ID ${existingStockCheck[0].product_id}`
      });
    }

    // Check if the product already has a different stock ID
    const [existingProductStock] = await db.query(
      'SELECT * FROM product_stocks WHERE product_id = ?',
      [product_id]
    );

    if (existingProductStock.length > 0 && existingProductStock[0].id !== id) {
      return res.status(400).json({ 
        error: `Product ID ${product_id} is already assigned to Stock ID ${existingProductStock[0].id}`
      });
    }
    ...
    if (existingProductStock.length > 0) {
      ...
      const newQuantity = existingProductStock[0].quantity + parseInt(quantity);
      await db.query(
        'UPDATE product_stocks SET quantity = ?, last_updated = NOW() WHERE product_id = ?',
    ...
    await db.query(
      'INSERT INTO product_stocks (id, product_id, quantity, last_updated) VALUES (?, ?, ?, NOW())',
